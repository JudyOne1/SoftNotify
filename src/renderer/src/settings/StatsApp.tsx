...
export default function StatsApp(): React.JSX.Element {
  ...
  /** react-activity-calendar 要求从今天往前连续的日期序列，level 0-4 */
    ...
    <div className="min-h-screen bg-background p-6 text-foreground">
      <div className="mx-auto max-w-[520px]">
        <div className="mb-4 flex items-center justify-between">
          <h1 className="text-lg font-bold">打卡统计</h1>
        ...
        <div className="rounded-lg bg-card p-4 shadow-[var(--neu-raised)]">
          {stats ? (
            <ActivityCalendar
              data={calendar}
              theme={{
        ...
        <h2 className="mb-2 mt-5 text-xs font-semibold uppercase tracking-wider text-muted-foreground">今日进度</h2>
        <div className="flex flex-col gap-2.5">
          ...
          {otherItems.length > 0 && (
            <div className="rounded-lg bg-card p-3 text-[13px] text-muted-foreground shadow-[var(--neu-raised-sm)]">
              其他：{' '}
              {otherItems
                .map((r) => `${r.name} ${stats?.todayByItem[r.id] ?? 0} 次`)
                .join(' · ')}
            </div>
          )}
          {stats?.todayTotal === 0 && (
          ...
          {goalItems.length === 0 && otherItems.length === 0 && (
            <div className="rounded-lg p-4 text-center text-[13px] text-muted-foreground shadow-[var(--neu-inset-sm)]">
              在设置的提醒项里填「每日目标」即可在这里看进度
            </div>
          )}
        </div>
      </div>
