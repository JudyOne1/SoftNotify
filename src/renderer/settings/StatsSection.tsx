...
/** 回顾视图：活跃图 + 今日进度 + 导出。theme 为主窗口当前生效主题。 */
export default function StatsSection({ theme }: { theme: 'light' | 'dark' }): React.JSX.Element {
  ...
  const allItems = [...(config?.reminders ?? []), ...(config?.schedules ?? [])]
  const goalItems = allItems.filter((item) => item.enabled && item.dailyGoal)
  const otherItems = allItems.filter((item) => item.enabled && !item.dailyGoal)
    ...
    <div className="flex flex-col gap-4">
      ...
      <div className="flex flex-col gap-2.5">
        ...
        {stats && stats.todayFocus > 0 && (
          <div className="flex items-center gap-2 rounded-lg bg-card p-3 text-[13px] text-muted-foreground shadow-[var(--neu-raised-sm)]">
            <Timer className="size-4 text-primary" />
            今日专注 {stats.todayFocus} 个
          </div>
        )}
        {stats?.todayTotal === 0 && (
